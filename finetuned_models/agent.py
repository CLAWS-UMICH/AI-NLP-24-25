...
def default_openai_chat(messages):
    # Format the conversation into a single instruction string
    instruction = ""
    ...
    for msg in messages:
        ...
        if role == "user":
            ...
            instruction += f"assistant: {content}\n"
            
        # Add function call results in exact format from training data
        elif role == "system" and "Tool '" in content:
            # Extract tool name and result from system message
            tool_start = content.find("Tool '") + 6
            tool_end = content.find("'", tool_start)
            tool_name = content[tool_start:tool_end]
            
            result_start = content.find("returned: ") + 10
            result = content[result_start:]
            
            instruction += f"assistant: Function Call: {tool_name}([])\nResult: {result}\n"
...
class Agent:
    ...
    def format_conversation_history(self):
        ...
        print("CONVERSATION HISTORY CONVERSATION HISTORY CONVERSATION HISTORY CONVERSATION HISTORY CONVERSATION HISTORY CONVERSATION HISTORY CONVERSATION HISTORY CONVERSATION HISTORY ")
    ...
    def execute_tool(self, tool_name, params):
        ...
        # Fix: Check if params is empty dict and call function appropriately
        if not params:
            result = tool.function()  # Call with no parameters
        else:
            result = tool.function(**params)  # Call with unpacked parameters
    ...
    def process_response(self):
        try:
            ...
            print("RESPONSERESPONSRESPREOSNRESPRONESRPSEON")
            print(response)
            print("RESPONSERESPONSRESPREOSNRESPRONESRPSEON")
...
def get_vitals(input_text=None):
    ...
    vitals = {
        "heart_rate": "1 bpm",
        "blood_pressure": "1/1 mmHg",
        "temperature": "1°F", 
        "respiration_rate": "1 breaths per minute"
    }
